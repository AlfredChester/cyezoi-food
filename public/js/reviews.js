  ...
  return `
    <div class="review-item">
      ...
      <div class="review-actions">
        <button class="like-btn ${isLiked ? "liked" : ""}" data-like-id="${
    review._id
  }">${isLiked ? "❤️" : "🤍"} ${review.likesCount || 0}</button>
        ${
          isOwnReview
            ? `<button class="edit-btn" data-edit-id="${review._id}">✏️ 编辑</button>`
            : ""
        }
        ${
          isOwnReview || isAdmin
            ? `<button class="delete-review-btn" data-del-id="${review._id}">🗑️ 删除</button>`
            : ""
        }
      </div>
    </div>
...
App.prototype.renderReviewsPagination = function (pagination) {
  ...
  } data-page="${pagination.currentPage - 1}">上一页</button>`;
  ...
  } data-page="${pagination.currentPage + 1}">下一页</button>`;
