        ...
        ${announcements
          ...
          <div class="ann-card" data-id="${a._id}">
            <div class="ann-card-header">
              ...
              ${
                a.pinned
                  ? '<span class="ann-badge ann-badge-pinned">置顶</span>'
                  : ""
              }
            </div>
            <div class="ann-card-meta">
              <span class="ann-author">${
                a.createdBy ? escapeHtml(a.createdBy.username) : "系统"
              }</span>
              <span class="ann-date">${new Date(a.createdAt).toLocaleDateString(
                "zh-CN"
              )}</span>
            </div>
            <div class="ann-card-content">${escapeHtml(
...
export function showAnnouncementModal(ann) {
  ...
  if (!modal) {
    ...
    document
      .getElementById("announcementModalClose")
  ...
  ).innerHTML = `<div class="ann-full">${escapeHtml(ann.content)}</div>`;
...
export function showAdminAnnouncementModal() {
  ...
  if (!modal) {
    ...
    // 绑定事件
    document
      .getElementById("adminAnnounceModalClose")
      .addEventListener("click", () => {
        modal.style.display = "none";
      });
    document
      .getElementById("cancelCreateAnnounce")
      .addEventListener("click", () => {
        modal.style.display = "none";
      });
    document
      .getElementById("createAnnounceTab")
      .addEventListener("click", () => switchTab("create"));
    document
      .getElementById("manageAnnounceTab")
      .addEventListener("click", () => switchTab("manage"));
    document
      .getElementById("createAnnounceForm")
      .addEventListener("submit", handleCreateAnnounce);
  ...
  const html = announcements
    ...
    <div class="manage-ann-item">
      <div class="manage-ann-info">
        <h4>${escapeHtml(ann.title)} ${
        ann.pinned ? '<span class="badge">置顶</span>' : ""
      } ${
        !ann.published ? '<span class="badge unpublished">未发布</span>' : ""
      }</h4>
      ...
      <div class="manage-ann-actions">
        ...
        <button onclick="deleteAnnouncement('${
          ann._id
        }')" class="btn-small danger">删除</button>
      </div>
    </div>
  `
    )
