...
App.prototype.bindEvents = function () {
  ...
  if (todayBtn)
    todayBtn.addEventListener("click", () => this.getTodayRecommendation());
  const addFoodBtn = document.getElementById("addFoodBtn");
  if (addFoodBtn)
    addFoodBtn.addEventListener("click", () => this.openAddFoodModal());
  ...
  document.querySelectorAll(".view-btn").forEach((btn) => {
    btn.addEventListener("click", () =>
      this.openReviewModal(btn.dataset.id, btn.dataset.name)
    );
  });
  ...
  if (addFoodForm) {
    ...
    if (cancel)
      cancel.addEventListener("click", () => this.closeAddFoodModal());
  ...
  if (editModal)
    editModal.addEventListener("click", (e) => {
      if (e.target === editModal) this.closeEditReviewModal();
    });
  ...
  document.querySelectorAll("[data-like-id]").forEach((btn) => {
    btn.addEventListener("click", () =>
      this.toggleReviewLike(btn.dataset.likeId)
    );
  });
  document.querySelectorAll("[data-edit-id]").forEach((btn) => {
    btn.addEventListener("click", () =>
      this.openEditReviewModal(btn.dataset.editId)
    );
  });
  ...
  if (pagination) {
    pagination.querySelectorAll("button").forEach((btn) => {
      ...
      if (page)
        btn.addEventListener("click", () =>
          this.loadReviews(this.currentFoodId, Number(page))
        );
    });
  }
};
