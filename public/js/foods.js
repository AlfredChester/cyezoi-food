  ...
  return `
    <div class="container">
      ...
      <div class="controls">
        ...
        ${
          this.currentUser.role === "admin"
            ? '<button class="admin-announce-btn" id="adminAnnounceBtn">📢 管理公告</button>'
            : ""
        }
      </div>
  ...
  return `
    <div class="food-card" data-category="${category}">
      ...
      <div class="food-actions">
        <button class="view-btn" data-id="${id}" data-name="${name.replace(
    /'/g,
    ""
  )}">查看 / 评价</button>
        ${
          canEdit
            ? `<button class="edit-food-btn" data-id="${id}">编辑</button>`
            : ""
        }
        ${
          canDelete
            ? `<button class="danger delete-food-btn" data-id="${id}">删除</button>`
            : ""
        }
      </div>
    </div>
