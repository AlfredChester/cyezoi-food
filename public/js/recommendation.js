...
App.prototype.showRecommendationModal = function (
  food,
  message,
  isNewRecommendation
) {
    ...
    <div class="modal-content recommendation-modal">
      ...
      <div class="recommendation-content">
        <div class="recommended-food">
          ...
          <div class="food-info">
            <h3>${food.name}</h3>
            <p class="food-category">${this.categoryEmojiMap[food.category]} ${
    food.category
  }</p>
        ...
        <div class="recommendation-rating">
          ...
          <span class="rating-text">${Number(food.averageRating || 0).toFixed(
            1
      ...
      <div class="recommendation-actions">
        ...
        <button class="btn-primary" data-view-id="${
          food._id
        }" data-view-name="${food.name}">查看详情</button>
      </div>
    </div>`;
  ...
  modal.addEventListener("click", (e) => {
    ...
    if (e.target.hasAttribute("data-history")) {
      this.showRecommendationHistory();
    }
    if (e.target.hasAttribute("data-view-id")) {
      this.openReviewModal(
        e.target.getAttribute("data-view-id"),
...
App.prototype.showHistoryModal = function (recommendations) {
  ...
  modal.addEventListener("click", (e) => {
    if (e.target === modal || e.target.hasAttribute("data-close"))
      modal.remove();
    if (e.target.hasAttribute("data-open-id")) {
      this.openReviewModal(
        e.target.getAttribute("data-open-id"),
        e.target.getAttribute("data-open-name")
      );
      modal.remove();
    }
  });
