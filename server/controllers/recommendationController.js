...
exports.today = async (req, res) => {
  try {
    ...
    if (allFoods.length === 0)
      return res
        .status(404)
        .json({ success: false, message: "暂无美食数据，请先添加一些美食！" });
    ...
    let availableFoods = allFoods.filter(
      (f) => !recentFoodIds.includes(f._id.toString())
    );
    ...
    const selectedFood =
      availableFoods[Math.abs(seedHash) % availableFoods.length];
    ...
    res
      .status(500)
      .json({ success: false, message: "获取今日推荐失败，请稍后重试" });
  }
};
...
exports.history = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "获取推荐历史失败" });
  }
};
