...
exports.create = async (req, res) => {
  try {
    ...
    if (!content || !rating)
      return res
        .status(400)
        .json({ success: false, message: "评论内容和评分不能为空" });
    if (rating < 1 || rating > 5)
      return res
        .status(400)
        .json({ success: false, message: "评分必须在1-5星之间" });
    const food = await Food.findById(foodId);
    if (!food)
      return res.status(404).json({ success: false, message: "美食不存在" });
    ...
    if (e.code === 11000)
      return res
        .status(400)
        .json({ success: false, message: "您已经评论过这个美食了" });
    res
      .status(500)
      .json({ success: false, message: "添加评论失败，请稍后重试" });
  }
};
...
exports.update = async (req, res) => {
  try {
    ...
    if (String(review.userId) !== String(req.user._id))
      return res
        .status(403)
        .json({ success: false, message: "只能修改自己的评论" });
    ...
    if (food) await food.calculateRating();
...
exports.remove = async (req, res) => {
  try {
    ...
    if (!isAuthor && !isAdmin)
      return res
        .status(403)
        .json({ success: false, message: "没有权限删除此评论" });
