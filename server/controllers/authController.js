...
exports.register = async (req, res) => {
  try {
    ...
    if (!username || !email || !password) {
      return res
        .status(400)
        .json({ success: false, message: "请填写完整的用户名、邮箱和密码" });
    }
    if (password.length < 6) {
      return res
        .status(400)
        .json({ success: false, message: "密码至少需要6位字符" });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "服务器内部错误，请稍后重试" });
  }
};
...
exports.login = async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res
        .status(400)
        .json({ success: false, message: "请输入邮箱和密码" });
    }
